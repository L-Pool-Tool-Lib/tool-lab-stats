...
while [[ $(date --date "$end_date +$GATSBY_STEP_SIZE_IN_DAYS days" '+%s') -le $(date --date "$GATSBY_RANGE_END_DATE" '+%s') ]]; do
    ...
    curl --location "https://$MT_API_URL/library/orgLoan/exportAggregateLoanReport" \
        ...
        --compressed \
        --output "data/${AGGREGATE_ATTRIBUTE}/${start_date}-to-${end_date}.csv"
